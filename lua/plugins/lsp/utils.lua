...
local function keymaps(bufnr)
    ...
    vim.api.nvim_buf_set_keymap(bufnr, "n", "gl", '<cmd>lua vim.diagnostic.open_float(0, {scope = line, border = "none"})<CR>', opts )
...
M.on_attach = function(client, bufnr)
    if client.name == "tsserver" then
        client.resolved_capabilities.documment_formatting = false
    end
